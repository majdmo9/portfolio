  ...
  return (
    <div key={skill} className="flex w-full gap-4 items-center justify-start">
      ...
      <div className="flex flex-col">
        <p className={boldTitle ? "font-bold lg:text-lg text-sm" : "font-medium text-sm lg:text-lg leading-none w-fit"} dir={dir}>
          {t(Skills_Data[skill].title)}
          {boldTitle ? "" : ":"}
        </p>
        <p className="lg:text-lg text-sm font-demi">{t(String(Skills_Data[skill].description ?? ""))}</p>
      </div>
    </div>
  );
};
